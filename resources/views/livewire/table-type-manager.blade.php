<div class="space-y-4">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">🪑 Masa Tipleri</h1>
            <p class="text-xs text-slate-500 mt-0.5">Bir masaya kaç çiçek girdiğini tanımla. Siparişte masa sayısı ile çarpılır.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse ($tableTypes as $type)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                @if ($type->image_url)
                    <img src="{{ $type->image_url }}" class="w-full h-32 object-cover" alt="{{ $type->name }}">
                @endif
                <div class="p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <div class="font-semibold text-slate-900">{{ $type->name }}</div>
                            <div class="text-xs text-slate-500">{{ $type->sponge_count }} sünger / masa</div>
                        </div>
                    ...
                    <ul class="text-sm text-slate-700 mt-3 space-y-1">
                        @foreach ($type->flowerLines as $line)
                            <li class="flex justify-between"><span>{{ $line->flowerType->name }}</span><span class="font-mono font-medium">{{ rtrim(rtrim(number_format($line->quantity_per_table, 2, ',', '.'), '0'), ',') }} {{ $line->flowerType->unit }}</span></li>
                        @endforeach
                    </ul>
                </div>
            </div>
        @empty
            <div class="md:col-span-2 lg:col-span-3 bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center text-slate-400">
                Henüz masa tipi yok. Üstteki <strong>+ Yeni Masa Tipi</strong> butonuyla ekleyebilirsin.
            </div>
        @endforelse
    </div>

    <x-modal :title="$editingId ? 'Masa Tipi Düzenle' : 'Yeni Masa Tipi'" max-width="max-w-3xl">
        <form wire:submit="save" id="table-form" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="sm:col-span-2">
                    <label class="block text-xs font-semibold text-slate-600 mb-1">Masa Tipi Adı *</label>
                    <input type="text" wire:model="name" autofocus class="w-full rounded-lg border-slate-300 focus:border-brand-500 focus:ring-brand-500" placeholder="örn. Standart Masa - 1 Sünger">
                    @error('name') <div class="text-rose-600 text-xs mt-1">{{ $message }}</div> @enderror
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1">Sünger Sayısı</label>
                    <input type="number" min="0" wire:model="sponge_count" class="w-full rounded-lg border-slate-300 focus:border-brand-500 focus:ring-brand-500">
                </div>
            ...
            <div class="border border-slate-200 rounded-xl p-4 bg-slate-50">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="font-semibold text-slate-700 text-sm">Bir Masada Kullanılacak Çiçekler</h3>
                    ...
                    @foreach ($lines as $i => $line)
                        <div class="grid grid-cols-12 gap-2 items-center bg-white p-2 rounded-lg border border-slate-200">
                            ...
                            <div class="col-span-3">
                                <input type="number" step="0.01" min="0.01" wire:model="lines.{{ $i }}.quantity_per_table" class="w-full rounded-lg border-slate-300 text-sm" placeholder="adet/masa">
                                @error("lines.$i.quantity_per_table") <div class="text-rose-600 text-xs mt-1">{{ $message }}</div> @enderror
                            </div>
        ...
        <x-slot:footer>
            ...
            <button type="submit" form="table-form" class="bg-brand-600 hover:bg-brand-700 text-white rounded-lg px-5 py-2 font-medium shadow-sm text-sm">
